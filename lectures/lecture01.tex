...
\item We add randomness as mashing bit
...
$\longrightarrow$ Find out how exactly this gives a random bit

\subsection{Example: Millionaires' Problem}
...
output $x \geq y$ \hspace{2cm} output $x \geq y$

This is easy with a trusted entity $T$.

\begin{center}
$A \longrightarrow |T| \longleftarrow B$ \\
$b \gets (x \geq y)$
\end{center}
...
$\longrightarrow$ cc: lower levels can see everything above them
...
$\text{split}(b)$: $\{0,1,2,...,p-1\}$

\hspace{1cm} $x_1 \xleftarrow{\$} \Z_p$ \\
\hspace{1cm} $x_2 \xleftarrow{\$} \Z_p$ \\
\hspace{1cm} $x_3 \xleftarrow{\$} \Z_p$ s.t. $x_1 + x_2 + x_3 \equiv b \pmod{p}$ \\
\hspace{1cm} return $(x_1, x_2, x_3)$
...
\textbf{Correctness:} Every party receives the output as specified by the function they are evaluating

\textbf{Input Independence:} Inputs of corrupted or faulty parties do not depend on input of correct parties.

\textbf{Fairness:} All parties either receive an output or no party receives an output \\
